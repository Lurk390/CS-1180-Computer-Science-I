...
public class P2_CreditCardValidation {
    ...
    // calculates the sum of every other digit from right to left starting at the
    // rightmost digit
    public static int oddSum(long creditCardInput) {
        int sumOdd = 0;

        while (creditCardInput > 0) {
            sumOdd += creditCardInput % 10; // returns the remainder when the card number is divided by 10 (last digit)
                                            // and adds it to the overall sum
            creditCardInput /= 100; // skips two digits from right to left
        }
        return sumOdd;
    }

    // calculates sum of every other digit right to left starting from second
    // rightmost digit
    public static int evenSum(long creditCardInput) {
        int sumEven = 0;
        while (creditCardInput > 0) {
            double evenSingleDigit = (2 * Math.floor((creditCardInput % 100) / 10)); // returns the remainder when the
                                                                                     // card number is divided by 100
                                                                                     // (last two digits), then divide
                                                                                     // by 10 then floor it for the
                                                                                     // first digit (ex. 26 = 2)
 ...
 * numbers.
 * The algorithm is useful to determine if a card number is entered correctly or
 * if a credit
 * card is scanned correctly by a scanner. Almost all credit card numbers are
 * generated
 * following this validity check, commonly known as the Luhn check or the Mod 10
 * check,
 * which can be described as follows (for illustration, consider the card number
 * 4388576018402626):
 * 
 * 1. Double every second digit from right to left. If doubling of a digit
 * results in a two-digit
